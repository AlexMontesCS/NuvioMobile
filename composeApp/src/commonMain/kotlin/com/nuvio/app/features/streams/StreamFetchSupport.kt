...
/**
 * Fetches `/stream` results from an installed addon, supporting both regular
 * JSON responses and incremental NDJSON responses.
 *
 * When the addon answers with `Content-Type: application/x-ndjson`, every
 * non-blank line carries one complete `{"streams":[...]}` object. Each decoded
 * batch is published through [onIntermediateGroup] as soon as it arrives (with
 * `isLoading = true`), so the UI can render streams while later scrapes are
 * still running. Streams accumulate across batches; duplicates are dropped.
 *
 * Regular JSON responses behave exactly like before: the whole payload is
 * buffered, parsed once, and returned without intermediate calls.
 *
 * Returns the final group for the addon (`isLoading = false`) containing
 * every decoded stream. Throws when the response is blank or malformed.
 */
internal suspend fun fetchAddonStreamsIncrementally(
        ...
        onLine = { line ->
            ...
            if (isNdjson && line.isNotBlank()) {
                ...
                if (batch.isNotEmpty()) {
                    ...
                    // A line that already contains every stream seen so far is a
                    // cumulative snapshot, so its ordering is authoritative —
                    // adopt it wholesale instead of appending, otherwise the
                    // addon's ordering (e.g. quality-grouped) is lost behind
                    // first-arrival order. Lines that only carry new streams are
                    // per-source deltas and keep appending.
    ...
    return when {
        // At least one NDJSON batch was decoded: the accumulated streams are
        // the final result.
        ...
        // Regular JSON: strict single-document parse, same as before.
        !isNdjson -> {
        ...
        // Empty NDJSON response (no batches at all) is a valid empty result.
